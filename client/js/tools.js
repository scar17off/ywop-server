...
eventSys.once(e.misc.toolsRendered, () => {
	...
	addTool(new Tool("Circle", cursors.circle, PLAYERFX.NONE, RANK.USER, function (tool) {
		...
		function tick() {
			for (let i = queue.length - 1; i >= 0; i--) {
				let pixel = queue[i];
				if (_main.misc.world.setPixel(pixel[0], pixel[1], player.selectedColor)) {
					queue.splice(i, 1);
				}
			}
	...
	addTool(new Tool('Pencil', cursors.pencil, PLAYERFX.RECT_SELECT_ALIGNED(1), RANK.USER, function (tool) {
		var lastX, lastY;
		tool.factor = 0.2;
	...
	addTool(new Tool('Pixel log', cursors.pixellog, PLAYERFX.NONE, RANK.MODERATOR, function (tool) {
		tool.setEvent("mousedown mouvemove", mouse => {
			let x = mouse.tileX;
			let y = mouse.tileY;
			windowSys.addWindow(new GUIWindow(`Pixel ${x}, ${y}`, {
				closeable: true
			}, function (win) {
	...
	addTool(new Tool('Cut', cursors.cut, PLAYERFX.NONE, RANK.MODERATOR, function (tool) {
		function drawText(ctx, str, x, y, centered) {
			ctx.strokeStyle = "#000000", ctx.fillStyle = "#FFFFFF", ctx.lineWidth = 2.5, ctx.globalAlpha = 0.5;
			if (centered) {
				x -= ctx.measureText(str).width >> 1;
	...
	addTool(new Tool('Brush', cursors.brush, PLAYERFX.NONE, RANK.MODERATOR, function (tool) {
		tool.brDiameter = 5;
		var lastX, lastY;
	...
	windowSys.addWindow(new GUIWindow('Brush diameter', {}, function (win) {
