...
eventSys.on(e.net.world.setId, id => {
	...
	setInterval(function () {
		if (!options.showPlaceBucket) return;
		const placeBucket = net.protocol.placeBucket;
		let allowance = Math.floor(placeBucket.allowance).toString();
		placeBucket.canSpend(0);
		let rate = placeBucket.rate;
