...
var chunkSize = server.config.databaseStoreSize; // stored information size in chunks. minimum: 2

if(chunkSize === 1) {
	console.log("CHUNKSIZE < 1. EXITING...");
	exit();
};
...
function get_chunk(worldName, x, y) {
	...
	var regX = Math.floor(x / chunkSize);
    var regY = Math.floor(y / chunkSize);
	
    if (
        (x < -server.config.worldborder.x || x >= server.config.worldborder.x) ||
        (y < -server.config.worldborder.y || y >= server.config.worldborder.y)
    ) {
        return new Uint8Array(chunkSize * chunkSize * 3).fill(0);
    };
    ...
    if (clusterSize < chunkSize * chunkSize * 3) {
        return null;
    }
...
function set_chunk(worldName, x, y, cdata) { // cdata = 16*16*3 RGB
    ...
    if (clusterSize < chunkSize * chunkSize * 3) {
        ...
        file_write(fd, clusterSize, Buffer.alloc(chunkSize * chunkSize * 3 - clusterSize));
    }
	var lookup = 3 * ((x & 31) + (y & 31) * 32);
	var chunkpos = file_read(fd, lookup, 3);
	chunkpos = chunkpos[2] * 16777216 + chunkpos[1] * 65536 + chunkpos[0] * 256;
